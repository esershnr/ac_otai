...
function script.update(dt)
    ...
    if player.engineLifeLeft < 1 then
        if totalScore > highestScore then
            ...
            ac.sendChatMessage("Sollamaca: " .. totalScore .. " puan kastı.")
        end
        totalScore = 0
        comboMeter = 1
        return
    ...
    elseif player.wheelsOutside > 0 then
        ...
        addMessage("Araba dışarda kalıyor aga, toparla!", -1)
    ...
    if player.speedKmh < requiredSpeed then
        if dangerouslySlowTimer > 3 then
            if totalScore > highestScore then
                highestScore = math.floor(totalScore)
                ac.sendChatMessage("bici " .. totalScore .. " puana oynadı.")
            end
            totalScore = 0
            comboMeter = 1
        else
            if dangerouslySlowTimer == 0 then
                addMessage("Hızlan moruk!", -1)
            end
        end
        ...
        if car.pos:closerToThan(player.pos, 10) then
            ...
            if not drivingAlong then
                ...
                if not state.nearMiss and car.pos:closerToThan(player.pos, 3) then
                    ...
                    if car.pos:closerToThan(player.pos, 2.5) then
                        ...
                        addMessage("Yalama bonusu: helaaal!", 0)
                    end
                end
            end

            if car.collidedWith == 0 then
                addMessage("Perte çıkardın!", -1)
                state.collided = true

                if totalScore > highestScore then
                    highestScore = math.floor(totalScore)
                    ac.sendChatMessage("Toplamda " .. totalScore .. " puan skorladın.")
                end
                totalScore = 0
                comboMeter = 1
            end
            ...
            if not state.overtaken and not state.collided and state.drivingAlong then
                ...
                if posDot < -0.5 and state.maxPosDot > 0.5 then
                    ...
                    addMessage("Sollamacaa", comboMeter > 20 and 1 or 0)
...
local speedWarning = 0
    function script.drawUI()
        ...
        ui.beginTransparentWindow("overtakeScore", vec2(20, 500), vec2(400 * 0.5, 400 * 0.5))
        ui.beginOutline()

        ui.pushStyleVar(ui.StyleVar.Alpha, 1 - speedWarning)
        ui.pushFont(ui.Font.Main)
        ui.text("En yüksek gördüğün: " .. highestScore .. " puan")
        ui.popFont()
        ui.popStyleVar()
